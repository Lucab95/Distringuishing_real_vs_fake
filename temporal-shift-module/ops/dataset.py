...
class VideoRecord(object):
    def __init__(self, row,n_classes):
        self._data = row
    ...
    def label(self):
        if self.n_classes==2:
            # print("data",self._data[0])
            if self._data[0].startswith("n"):
                return 0
            else:
                return 1
        elif self.n_classes==6:
            name = self._data[0].split("\\")[0]
            if name.endswith("a"):
                return 0
            if name.endswith("c"):
                return 1
...
class TSNDataSet(data.Dataset):
    def __init__(self, root_path, list_file,
                 ...
                 remove_missing=False, dense_sample=False, n_classes=12):
    ...
    def _parse_list(self):
        ...
        self.video_list = [VideoRecord(item,self.n_classes) for item in tmp]
    ...
    def __getitem__(self, index):
        ...
        while not os.path.exists(full_path):
            ...
            if self.image_tmpl == 'flow_{}_{:05d}.jpg':
                ...
                file_name = self.image_tmpl.format(1)
                full_path = os.path.join(self.root_path, record.path, file_name)
        # print(record.path, record.num_frames, record.label)
        # if self.n_classes == 2:
        #     if record.path.startswith('n'):
        #         record["label"] = 0
        #     else:
        #         record["label"] = 1
