...
parser.add_argument('--dataset', type=str, default='sasefe')

# may contain splits
parser.add_argument('--weights', type=str, default="./checkpoint/TSM_2_emotions_sasefe_RGB_resnet50_avg_segment8_e60/ckpt.best.pth.tar")
...
# for true test
parser.add_argument('--test_list', type=str, default="video/val_videofolder_test.txt")
...
def plot_confusion_matrix(y_true, y_pred, classes,
    ...
    if not title:
        if normalize:
            title = 'Normalized confusion matrix_'
        else:
    ...
    if normalize==True:
        title = f"tsm_{n_classes}_emotions_normalized_{cls_acc}%_acc"
    else:
        title = f"tsm_{n_classes}_emotions_{cls_acc}%_acc"
...
for this_weights, this_test_segments, test_file in zip(weights_list, test_segments_list, test_file_list):
    ...
    data_loader = torch.utils.data.DataLoader(
            TSNDataSet(root_path, test_file if test_file is not None else val_list, num_segments=this_test_segments,
                       ...
                       ]), dense_sample=args.dense_sample, n_classes=n_classes),
            batch_size=args.batch_size, shuffle=False,
            num_workers=args.workers, pin_memory=True,
    )

    if args.gpus is not None:
        devices = 1 #[args.gpus[i] for i in range(args.workers)]
    else:
...
fig = plt.figure()
plt.matshow(cf)
plt.title('Problem 1: Confusion Matrix TSM 12emt')
plt.colorbar()
...
classes = int2emo.values()
print(classes)
cls_acc= '{:.02f}'.format(np.mean(cls_acc) * 100)
x,fig= plot_confusion_matrix(video_labels, video_pred, classes, normalize=False, cls_acc= cls_acc)
x,fig= plot_confusion_matrix(video_labels, video_pred, classes, normalize=True, cls_acc=  cls_acc)
